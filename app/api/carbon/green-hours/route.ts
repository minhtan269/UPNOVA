...
import { fetchGreenHoursFromProvider } from "@/lib/server/carbon-intensity-provider";
...
export async function GET(req: NextRequest): Promise<NextResponse> {
    ...
    try {
        const data = await fetchGreenHoursFromProvider(region);
        return NextResponse.json(data);
    } catch {
        return NextResponse.json({ error: "Failed to fetch green-hours data." }, { status: 502 });
    }
}
